...
- `IRobotController`, `IEnvironmentInfo`, `SuctionGripper`, `ColorArea` 등의 공개 API 호출
...
namespace CPS.ICPBL.Student
{
    ...
    public interface IPoseProvider
    {
        ...
        StationPose GetBoxBasePose(ProductClass productClass);
    }

    public interface IPalletizer
    {
        BoxSlotPose ReserveNextSlot(ProductClass productClass, int robotId, int taskId);
...
씬의 RobotA, RobotB, EnvironmentInfo, Gripper, ColorArea 등을 한 곳에서 관리한다. 파일 위치는 `Student/Bootstrap/StudentSceneReferences.cs`로 둔다.
...
`ConveyorSnapshot[]`와 `RobotSnapshot`을 받아 가장 처리할 가치가 높은 작업을 고른다.
...
5. Slice C의 PoseProvider에서 conveyor pick pose 요청
6. approachPos로 MoveArmTo()
7. actionPos로 MoveArmTo()
8. GripperAdapter.WaitUntilGraspReady()
9. TryGrip()
10. IsHolding 확인
11. retractPos로 MoveArmTo()
12. Slice D의 ColorClassifier로 색상 판정
13. 목적 station 결정: Normal=100, Abnormal=101
14. Conveyor lock 해제
15. 필요 시 CentralZone lock 해제 또는 다음 이동까지 유지
16. Box lock 요청
17. GoToOperatingStation(100 또는 101)
18. Slice C의 Palletizer에서 다음 slot 예약
19. slot.approachPos로 MoveArmTo()
20. slot.placePos로 MoveArmTo()
21. Release()
22. slot.retractPos로 MoveArmTo()
23. Palletizer.CommitSlot(taskId)
24. Box lock 해제
25. MissionResult success 반환
```
...
B는 색상 판정 로직을 직접 작성하지 않는다. D의 ColorClassifier만 호출한다.
...
public class PoseTable : MonoBehaviour, IPoseProvider
{
    ...
    public StationPose GetBoxBasePose(ProductClass productClass);
}
```
...
{
    public BoxSlotPose ReserveNextSlot(ProductClass productClass, int robotId, int taskId);
...
```csharp
BoxSlotPose slot = palletizer.ReserveNextSlot(productClass, robotId, taskId);
```

반환 조건:

- `productClass == Normal`이면 `stationId = 100`
- `productClass == Abnormal`이면 `stationId = 101`
...
7. `Palletizer.ReserveNextSlot()` 구현
...
`ColorArea.color`에서 읽은 색상을 받아 Normal/Abnormal을 판정한다.
...
- `productClass`: Normal / Abnormal / Unknown
- `reliable`: 판정 신뢰 여부
...
- [ ] Inspector에서 RobotA/B, EnvironmentInfo, Gripper, ColorArea reference를 연결할 수 있다.
...
| Color classification  | D              | B         | B가 ColorArea.color 전달                                                                |
...
B → C: ReserveNextSlot(productClass, robotId, taskId)
...
| D    | ColorArea.color 기반 거리 판정, defaultColor 재검사 |
...
- [ ] Inspector에서 RobotA/B, EnvironmentInfo, Gripper, ColorArea reference 연결 가능
- [ ] `ColorClassifier` 작성
- [ ] `ResourceLockManager` 작성
- [ ] `PathPlanner` 작성 또는 단순 정책 구현
- [ ] `DeadlockGuard` 작성 또는 timeout 정책 구현
- [ ] `TelemetryLogger` 작성
- [ ] ColorArea.color 판정 동작
...
| `Vision`    | `ColorArea.color` 기반 Normal/Abnormal 분류                 |
...
FleetManager
  ...
  │
  ├─ TaskAllocator
  │    └─ ConveyorSnapshot / RobotSnapshot
  │
  ├─ RobotAgent A
  │    └─ MissionExecutor A
  │
...
12. MissionExecutor가 ColorArea.color를 읽고 ColorClassifier에 분류를 요청한다.
...
4. `ColorArea.color`가 defaultColor이면 바로 판정하지 말고 재검사한다.
